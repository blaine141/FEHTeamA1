...
void (*interruptFuncs[32])();


DigitalIOPin::DigitalIOPin( FEHIO::FEHIOPin pin, bool interrupt, void (*func)())
{
    inInputMode = false;
    this->interrupt = interrupt;
    Initialize( pin, true);
    if(interrupt)
        interruptFuncs[_pin] = func;
}
...
void DigitalIOPin::Initialize( FEHIO::FEHIOPin pin, bool input )
{
    // store selected pin number in class
    _pin = pin;
    if(input && !inInputMode)
    {
        unsigned char trig = (unsigned char)FEHIO::FallingEdge;
        switch( GPIOPorts[ (int)_pin ] )
        {
            case PortA:
            {
                PORT_PCR_REG( PORTA_BASE_PTR, GPIOPinNumbers[ (int)_pin ] ) = ( 0 | PORT_PCR_MUX( 1 ) | PORT_PCR_PE_MASK | PORT_PCR_PS_MASK | ((this->interrupt)?PORT_PCR_IRQC(trig) | PORT_PCR_PFE_MASK:0));
                GPIOA_PDDR &= ~GPIO_PDDR_PDD( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                if(interrupt)
            ...
            case PortB:
            {
                PORT_PCR_REG( PORTB_BASE_PTR, GPIOPinNumbers[ (int)_pin ] ) = ( 0 | PORT_PCR_MUX( 1 ) | PORT_PCR_PE_MASK | PORT_PCR_PS_MASK | ((this->interrupt)?PORT_PCR_IRQC(trig) | PORT_PCR_PFE_MASK:0));
                GPIOB_PDDR &= ~GPIO_PDDR_PDD( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                if(interrupt)
            ...
            case PortC:
            {
                PORT_PCR_REG( PORTC_BASE_PTR, GPIOPinNumbers[ (int)_pin ] ) = ( 0 | PORT_PCR_MUX( 1 ) | PORT_PCR_PE_MASK | PORT_PCR_PS_MASK | ((this->interrupt)?PORT_PCR_IRQC(trig) | PORT_PCR_PFE_MASK:0));
                GPIOC_PDDR &= ~GPIO_PDDR_PDD( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                if(interrupt)
            ...
            case PortE:
            {
                PORT_PCR_REG( PORTE_BASE_PTR, GPIOPinNumbers[ (int)_pin ] ) = ( 0 | PORT_PCR_MUX( 1 ) | PORT_PCR_PE_MASK | PORT_PCR_PS_MASK | ((this->interrupt)?PORT_PCR_IRQC(trig) | PORT_PCR_PFE_MASK:0));
                GPIOE_PDDR &= ~GPIO_PDDR_PDD( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                if(interrupt)
...
void DigitalIOPin::Write(bool value)
{
    if(value)
    {
        Initialize(_pin,false);
        switch( GPIOPorts[ (int)_pin ] )
        {
            case PortA:
            {
                GPIOA_PDOR &= ~GPIO_PDOR_PDO( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                break;
            }
            case PortB:
            {
                GPIOB_PDOR &= ~GPIO_PDOR_PDO( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                break;
            }
            case PortC:
            {
                GPIOC_PDOR &= ~GPIO_PDOR_PDO( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                break;
            }
            case PortD:
            {
                GPIOD_PDOR &= ~GPIO_PDOR_PDO( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                break;
            }
            case PortE:
            {
                GPIOE_PDOR &= ~GPIO_PDOR_PDO( GPIO_PIN( GPIOPinNumbers[ (int)_pin ] ) );
                break;
            }
        }
